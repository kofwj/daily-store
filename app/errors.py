...
_COPY = {
    400: ("请求有误", "请求有问题，回到上一步再试。"),
    413: ("请求太大", "提交内容超出限制，缩小后再试。"),
    403: ("没有权限", "这个操作需要管理员权限。"),
    404: ("页面不存在", "页面不存在，从导航重新进。"),
    405: ("方法不支持", "只读页面，不能这样提交。"),
    500: ("服务器开小差了", "出了内部错误，数据不会丢。稍等刷新重试，或联系管理员。"),
    503: ("服务暂时不可用", "服务器忙，稍等刷新试试。"),
}


def _render(code: int) -> str:
    title, message = _COPY.get(code, ("出错了", "出了问题，回到上一步。"))
