...
开票取上月酬金申请（8 月考核对 7 月开票）；垫资、AI、奖惩用本月。
公式按已确认口径：
- 实际酬金 = 上月开票 + 房补 − 本月垫资
- 酬金得分 = 实际 / 目标，封顶 100
        ...
        out.append(
            {
                ...
                "invoice_total": float(inv.get("invoice_total") or 0),
                "housing": float(inv.get("housing") or 0),
...
def build_settlement_xlsx(conn, stores: Sequence[Any], as_of: date) -> bytes:
    ...
    lines = [
        ...
        ("开票", "取上月酬金开票申请的服务费+手续费。8 月考核对 7 月开票"),
        ("黄色格子", "开票/房补来自上月开票申请；没填仍可手改。实际搭载率另填；垫资有流水则系统回填"),
...
def _write_store_row(ws, row: int, manager: str, data: Dict[str, Any]) -> None:
    ...
    housing = float(data.get("housing") or 0)
    ws[f"F{row}"] = invoice_total or None
    ws[f"G{row}"] = housing or None
    ws[f"P{row}"] = None
    for col in ("F", "G", "P"):
